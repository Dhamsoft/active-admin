    ...
    def apply(router)
      ...
      router.instance_exec(@application.namespaces) do |namespaces|
        ...
        resources.each do |config|
		  ...
		  my_config = config
		  while my_config.belongs_to?
		    configs << my_config.belongs_to_config
		  	my_config = my_config.belongs_to_config.target
		  end

		  configs.each_with_index do |c, index|
		  	the_routes << route_definition_block.dup
		  	route_definition_block = Proc.new do
		    	# If its optional, make the normal resource routes
            	instance_eval &the_routes[index] if c.optional?

        		# Make the nested belongs_to routes
        		# :only is set to nothing so that we don't clobber any existing routes on the resource
          		resources c.target.underscored_resource_name.pluralize, :only => [] do
        			instance_eval &the_routes[index]
            	end
		  	end
		  end
