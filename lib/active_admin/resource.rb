...
module ActiveAdmin
  ...
  class Resource
    ...
    def belongs_to_root_target
    	resource = self
    	while resource.belongs_to?
    		resource = resource.belongs_to_config.target
    	end
    	resource
    end
    ...
    def belongs_to(*symbols)
      ...
      @belongs_to = Resource::BelongsTo.new(self, symbols.last, options)
      controller.belongs_to(*(symbols << options))
    end
